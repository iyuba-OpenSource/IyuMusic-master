...
public class NotificationBeforeReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        if (RuntimeManager.getApplication().getPlayerService().getCurArticleId() == 0 && !StudyManager.getInstance().getApp().equals("101")) {
            ...
            RuntimeManager.getApplication().getPlayerService().before();
            RuntimeManager.getApplication().getPlayerService().startPlay(StudyManager.getInstance().getCurArticle(), false);
