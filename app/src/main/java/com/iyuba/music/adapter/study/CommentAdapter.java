...
public class CommentAdapter extends RecyclerView.Adapter<RecycleViewHolder> {
    ...
    public void onBindViewHolder(RecycleViewHolder holder, int position) {
        ...
        if(null==comment.getUserName()||comment.getUserName().equals("")){
            commentViewHolder.name.setText(comment.getUserid());
        }else{
                ...
                public void onClick(View v) {
                    if (player.isPlaying()) {
                        ...
                        if (comment.getId() == playingComment) {
                            ...
                            playVoice("http://daxue." + ConstantManager.IYUBA_CN + "appApi/" + comment.getShuoshuo(), pos);// 播放
                        }
                    } else {
        ...
        commentViewHolder.pic.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (AccountManager.getInstance().checkUserLogin()) {
                    ...
                    context.startActivity(PersonalHomeActivity.buildIntent(context, Integer.parseInt(comments.get(pos).getUserid()), "", 0));
