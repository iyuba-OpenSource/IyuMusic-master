                        ...
                        for (int eventType = xmlPullParser.getEventType(); eventType != XmlPullParser.END_DOCUMENT; eventType = xmlPullParser.next()) {
                            switch (eventType) {
                                case XmlPullParser.START_TAG:
                                    ...
                                    if (word != null && "createDate".equals(nodeName)) {
                                        word.setCreateDate(xmlPullParser.nextText());
                                    }
                                    break;
...
//                                        Log.e("DictSynchroRequest", "createDate = " + word.getCreateDate());
                                        word.setViewCount("1");
                                        word.setIsdelete("0");
                                        words.add(word);
                                        word = new Word();
    ...
    public String generateUrl(String uid, int page) {
        String originalUrl = "http://word." + ConstantManager.IYUBA_CN + "words/wordListService.jsp";
        this.uid = uid;
