...
public class ScoreOperRequest {
    ...
    public static String generateUrl(String uid, int id, int type) {
        String originalUrl = "http://api." + ConstantManager.IYUBA_CN + "credits/updateScore.jsp";
        ...
        if (type == 40) {
            try {
                paras.put("flag", Base64.encodeToString(URLEncoder.encode(df.format(new Date(System.currentTimeMillis())), "UTF-8").getBytes(), Base64.DEFAULT));
