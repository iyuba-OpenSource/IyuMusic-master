...
public class OriginalSynView extends ScrollView implements
    ...
    public OriginalSynView(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            ...
            textColor = a.getColor(R.styleable.originalsynview_ori_textcolor, Color.WHITE);
            a.recycle();
        } else {
            defaultProperty();
        }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_CANCEL:
                Log.e("drawTimeLine", drawTimeLine + "");
                if (drawTimeLine) {
                    if (seekToCallBack != null) {
    ...
    private void initData() {
        ...
        for (int i = 0; i < size; i++) {
            textPage = new TextPage(context);
            textPage.setTextColor(textColor);
            textPage.setLineSpacing(3f, 1.3f);
            textPage.setWidth(ScreenUtils.getScreenWidth(context));
            textPage.setGravity(Gravity.CENTER_HORIZONTAL);
    ...
    public void synchroLanguage() {
        ...
        int size = originalList.size();
        for (int i = 0; i < size; i++) {
            // 歌词首位各有一个空的textview，更新数据从第二个数据开始
